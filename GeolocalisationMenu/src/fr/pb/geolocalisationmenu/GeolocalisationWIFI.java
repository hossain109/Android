...
// -----------------------------
public class GeolocalisationWIFI extends Activity implements OnClickListener {
    ...
    private void getEtatWiFi() {
        boolean lbWiFi = gestionnaireGeoloc
                .isProviderEnabled(LocationManager.NETWORK_PROVIDER);
        if (lbWiFi) {
            Toast.makeText(this, "WIFI disponible", Toast.LENGTH_LONG).show();
            textViewMessage.setText("WIFI disponible");
        } else {
            Toast.makeText(this, "WIFI indisponible", Toast.LENGTH_LONG).show();
            textViewMessage.setText("WIFI indisponible");
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (v == buttonDemarrerGeolocalisation) {
            ...
            gestionnaireGeoloc.requestLocationUpdates(
                    LocationManager.NETWORK_PROVIDER, 1000 * 60, 10, ecouteurGeoloc);
            ibEtatScanne = true;
        }
        ...
        if (v == buttonArreterGeolocalisation) {
            if (ibEtatScanne) {
                ...
                textViewMessage
                        .setText("Le service de géolocalisation via le WIFI n'était pas démarré");
            }
        }
